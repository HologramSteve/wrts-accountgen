...
def check_token(token):
  ...
  "x-session-id": "1e63d6ec-fa87-4b5b-816d-c08cbb091de6"
    ...
    if resjson == {'errors': [{'attribute': 'recaptcha_token', 'message': 'Los eerst de captcha op'}]}:
        ...
        client_key="e679f8168926a75a76ac81e051ddc4ec"
    )
    ...
    try:
        ...
        print(f"TOKEN: {token} | RESULT: INVALID")
        return 0
    
    print(f"TOKEN: {token} | RESULT: VALID")
    return 1

def check_tokens(credentials):
    print(f"Checking {len(credentials)} tokens...")
    valid = 0
    invalid = 0
    invalidtype = 0
