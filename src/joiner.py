...
def join(token, groupId, invite):
  ...
  "x-session-id": "1e63d6ec-fa87-4b5b-816d-c08cbb091de6"
    ...
    try:
        ...
        print(f"TOKEN: {token} | RESULT: NOT JOINED (invalid credentials)")
        return 0
    
    res = post(f"/groups/{groupId}/request_to_join", {"auto_join_secret": invite}, headers=headers)

    print(f"TOKEN: {token} | RESULT: JOINED")
    return 1

def massjoin(credentials, groupId, invite):
    print(f"Joining with {len(credentials)} tokens...")
    valid = 0
    invalid = 0
    invalidtype = 0
    ...
    def worker(index, token):
        result = join(token, groupId, invite)
...
while True:
    ...
    massjoin(tokens, groupid, invite)